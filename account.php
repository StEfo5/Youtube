...
<body>
	...
	<?php 
	    ...
	    $q = mysqli_query($con, "SELECT * FROM videos INNER JOIN users ON videos.user_id = users.id WHERE user_id = {$_GET['id']}");   	
	   	$str = $q -> fetch_assoc();
	...
	<div class="col-12 px-3" style="">	
		<div class="row">
			...
			<div class="col-10" style="height: 500px">
				...
				<div class="col-4">
					<form action="acc_update.php" method="GET">				
						<input type="" name="name" class="form-control" value="<?php echo $str['name'] ?>">
						<input type="" name="phone" class="form-control" value="<?php echo $str['phone'] ?>">
						<input type="" name="email" class="form-control" value="<?php echo $str['email'] ?>">
						<input type="" name="id" class="d-none form-control" value="<?php echo $str['id'] ?>">
						<button class="form-control btn btn-primary mt-2">Изменить</button>
					</form>	
					<form action="acc_delete.php?id=<?php echo $_GET['id'] ?>" method="GET">
